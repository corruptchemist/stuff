...
public class BlockDataHandler implements INBTSerializable<CompoundTag> {
   ...
   public Block getLeafType(BlockPos blockPos) {
      if (this.hasLeafData(blockPos)) {
         Optional<Block> leafBlock = BuiltInRegistries.BLOCK.getOptional(ResourceLocation.parse((String)this.deadLeaves.get(blockPos).getA()));
   ...
   public Block getSaplingType(BlockPos blockPos) {
      if (this.hasSaplingData(blockPos)) {
         Optional<Block> sapling = BuiltInRegistries.BLOCK.getOptional(ResourceLocation.parse(this.saplingReplacements.get(blockPos)));
   ...
   public void deserializeNBT(Provider provider, CompoundTag compoundTag) {
      ...
      if (version != 0) {
         ...
         for (int i = 0; i < rData.size(); i++) {
            this.reinforcedBlocks.add((BlockPos)NbtUtils.readBlockPos(rData, String.valueOf(i)).orElseThrow());
         }

         CompoundTag pData = compoundTag.getCompound("playerPlaced");

         for (int i = 0; i < pData.size(); i++) {
            this.placedBlocks.add((BlockPos)NbtUtils.readBlockPos(pData, String.valueOf(i)).orElseThrow());
         }
         ...
         for (int i = 0; i < sData.size(); i++) {
            ...
            this.saplingReplacements.put((BlockPos)NbtUtils.readBlockPos(sapling, "pos").orElseThrow(), sapling.getString("id"));
         }
         ...
         for (int i = 0; i < lData.size(); i++) {
            ...
            this.deadLeaves.put((BlockPos)NbtUtils.readBlockPos(leaf, "pos").orElseThrow(), new Pair(leaf.getString("id"), leaf.getInt("timeNoSupport")));
         }
      }
   }
}
