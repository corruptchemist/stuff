...
public class ParticleTexExtraRender extends ParticleTexFX {
   ...
   public void tickExtraRotations() {
      if (this.slantParticleToWind) {
         ...
         this.rotationPitch = this.rotationPitch + (float)(this.entityID % 10 - 5);
      }
   }
   ...
   public void render(VertexConsumer buffer, Camera renderInfo, float partialTicks) {
      ...
      for (int ii = 0; ii < renderAmount; ii++) {
         ...
         buffer.addVertex((float)(xx + (double)v3f[0].x), (float)(yy + (double)v3f[0].y), (float)(zz + (double)v3f[0].z))
            .setUv(u1, v1)
            .setColor(this.rCol, this.gCol, this.bCol, this.alpha)
            .setLight(i);
         buffer.addVertex((float)(xx + (double)v3f[1].x), (float)(yy + (double)v3f[1].y), (float)(zz + (double)v3f[1].z))
            .setUv(u1, v0)
            .setColor(this.rCol, this.gCol, this.bCol, this.alpha)
            .setLight(i);
         buffer.addVertex((float)(xx + (double)v3f[2].x), (float)(yy + (double)v3f[2].y), (float)(zz + (double)v3f[2].z))
            .setUv(u0, v0)
            .setColor(this.rCol, this.gCol, this.bCol, this.alpha)
            .setLight(i);
         buffer.addVertex((float)(xx + (double)v3f[3].x), (float)(yy + (double)v3f[3].y), (float)(zz + (double)v3f[3].z))
            .setUv(u0, v1)
            .setColor(this.rCol, this.gCol, this.bCol, this.alpha)
            .setLight(i);
      }
