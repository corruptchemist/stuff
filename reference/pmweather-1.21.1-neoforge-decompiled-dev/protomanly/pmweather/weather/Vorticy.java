...
public class Vorticy {
   ...
   public float getWidth() {
      return this.storm.is(StormTypes.CYCLONE) ? this.widthPerc * (float)this.storm.maxWidth : this.widthPerc * this.storm.width;
   }

   public float getDistance() {
      return this.storm.is(StormTypes.CYCLONE) ? this.distancePerc * (float)this.storm.maxWidth : this.distancePerc * this.storm.width;
   }

   public Vec3 getPosition() {
      float realDist = this.getDistance();
      return this.storm.position.add(new Vec3(Math.sin((double)this.angle) * (double)realDist, 0.0, Math.cos((double)this.angle) * (double)realDist));
   }
}
